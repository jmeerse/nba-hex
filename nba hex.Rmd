...
write.csv(df, file = "nbashots2022.csv")
...
```
#calculating league averages by hexbin
```{r}
...
# minimum of 10 attempts and have a z-score >= 0 (changed df name to _a, in case I wanted to work with hex_data later)
hex_data_a <- hex_data %>% filter(hex_attempts >= 10 & hex_data$z_score > 0 & !is.na(slugTeam)) 

```
#graphing
```{r}
#first graph isn't great
ggplot() +
  geom_polygon(data = hex_data_a, 
               aes(x = adj_x, y = adj_y, 
                   alpha = sqrt(z_score), 
                   fill = primary, 
...
#add titles, tweak plot dimensions, add court image
p <- ggplot() +
  geom_polygon(data = hex_data_a, 
               aes(x = adj_x, y = adj_y, 
                   alpha = sqrt(z_score), 
...
ggsave("HexChart22.png", p, width = 6, height = 6, dpi = 300, type = 'cairo') 
